...
cdef inline void _init_parent_record(ParentInfo* record) noexcept nogil:
    ...
    record.parent_vol_cst = -INFINITY
...
cdef struct ValueStackRecord:
    ...
    float64_t lower_bound
    float64_t upper_bound
    float64_t parent_won_volume

cdef class DepthFirstValueTreeBuilder(TreeBuilder):
    """Build a decision tree in depth-first fashion."""

    def __cinit__(self, ValueSplitter splitter, intp_t min_samples_split,
                  intp_t min_samples_leaf, float64_t min_weight_leaf,
                  intp_t max_depth, float64_t min_impurity_decrease):
        self.splitter = splitter
        self.min_samples_split = min_samples_split
    ...
    cpdef value_build(
        ...
        # Parameters
        cdef ValueSplitter splitter = self.splitter
        cdef intp_t max_depth = self.max_depth
        cdef intp_t min_samples_leaf = self.min_samples_leaf
        cdef float64_t min_weight_leaf = self.min_weight_leaf
        cdef intp_t min_samples_split = self.min_samples_split
        ...
        cdef float64_t right_child_min
        cdef float64_t right_child_max
        cdef float64_t left_won_volume
        cdef float64_t right_won_volume
        cdef bint is_leaf
        cdef bint first = 1
        cdef intp_t max_depth_seen = -1
        cdef int rc = 0
        ...
        with nogil:
            ...
            builder_stack.push({
                ...
                "lower_bound": -INFINITY,
                "upper_bound": INFINITY,
                "parent_won_volume": parent_won_volume
            })
            ...
            while not builder_stack.empty():
                ...
                parent_record.lower_bound = stack_record.lower_bound
                parent_record.upper_bound = stack_record.upper_bound
                parent_record.parent_vol_cst = stack_record.parent_won_volume
                ...
                if not is_leaf:
                    ...
                    left_won_volume = parent_won_volume
                    right_won_volume = parent_won_volume
                    if parent_won_volume > -INFINITY:
                        #left_won_volume = split.left_won_volume
                        #right_won_volume = split.right_won_volume
                        splitter.criterion.volume_won_left_and_right(&left_won_volume, &right_won_volume)
                    ...
                    builder_stack.push({
                        ...
                        "lower_bound": right_child_min,
                        "upper_bound": right_child_max,
                        "parent_won_volume": right_won_volume
                    })
                    ...
                    builder_stack.push({
                        ...
                        "lower_bound": left_child_min,
                        "upper_bound": left_child_max,
                        "parent_won_volume": left_won_volume
                    })
