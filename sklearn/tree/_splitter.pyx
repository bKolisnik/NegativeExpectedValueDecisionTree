...
from ._criterion cimport Criterion, ValueCriterion
...
cdef inline int node_split_value(
    ...
    cdef float64_t lower_bound = parent_record.lower_bound
    cdef float64_t upper_bound = parent_record.upper_bound
    cdef float64_t parent_volume_won = parent_record.parent_vol_cst
    ...
    cdef int value_init(
        ...
        for i in range(len(y)):
            for j in range(len(y[0])):
                self.expected_value[i][j] = -y[i][j] * deal_value[i][j]
